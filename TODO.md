...
2. **Normalize**. Strip characters of unnecessary elements, such as capitalization, accents, and punctuation. There will be a default normalizer function, but it will be possible to specify your own.
3. **Parse**. At this point, we will have text strings each corresponding to one word. The goal here will be to parse each word into morphemes. The default parser will parse each character as a morpheme, but one will be able to pass their own parser. This is important for most languages, as even the presence of a single diphthong could throw the program off.
